...
def _iter_textparts(obj: Any):
    """Deep traversal: gather strings from 'textPart' fields located inside arrays of dicts."""
    if isinstance(obj, dict):
        if "textPart" in obj and isinstance(obj["textPart"], str):
            yield obj["textPart"]
        for v in obj.values():
            yield from _iter_textparts(v)
    elif isinstance(obj, list):
        for el in obj:
            if isinstance(el, dict) and "textPart" in el and isinstance(el["textPart"], str):
                yield el["textPart"]
            else:
...
def _load_text_parts(textPartsPath: str) -> list[str]:
    ...
    if data is None:
        return []
    parts = [s for s in _iter_textparts(data) if isinstance(s, str) and s.strip()]
    cleaned = []
    for s in parts:
...
def main():
    ...
    ap.add_argument("--textPartsPath", default=None,
                help="Путь или URL к JSON с подготовленными частями текста (поле 'textPart').")
    ...
    try:
        ...
        # === РЕЖИМ ГОТОВЫХ ЧАСТЕЙ ===
        if args.textPartsPath:
            parts = _load_text_parts(args.textPartsPath)
            if not parts:
                eprint(f"[textParts] no usable 'textPart' found in: {args.textPartsPath}")
                sentences = []
            else:
        ...
        for idx, a in enumerate(audios, 1):
            ...
            while True:
                ...
                if attempts > MAX_RETRIES:
                    ...
                    if (not args.textPartsPath) and len(parts) > 1:
